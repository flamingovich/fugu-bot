...
import { Plus, Layout, Save, Trash2, Eye, ArrowLeft, FileCode, Wand2, Share2, Copy, Globe, Check } from 'lucide-react';
          ...
          {templates.map(tpl => (
            <div key={tpl.id} className="group bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
              <div className="p-6">
                ...
                <p className="text-xs text-slate-400 mb-6">Обновлено {new Date(tpl.updatedAt).toLocaleDateString()}</p>
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => navigate(`/view/${tpl.id}`)} className="bg-slate-900 text-white px-3 py-2.5 rounded-xl text-sm font-bold">Просмотр</button>
                  <button onClick={() => navigate(`/editor/${tpl.id}`)} className="bg-white border border-slate-200 text-slate-700 px-3 py-2.5 rounded-xl text-sm font-bold">Изменить</button>
                </div>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-8 flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">{initialTemplate ? 'Редактирование' : 'Новая страница'}</h1>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-6">
          <section className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
            <h2 className="text-lg font-bold mb-6 flex items-center gap-2">Настройки файла</h2>
            <div className="space-y-4">
              <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Название проекта" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none" />
              <div className="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center bg-slate-50 relative">
                <input type="file" accept=".html" onChange={handleFileUpload} className="absolute inset-0 opacity-0 cursor-pointer" />
                <p className="text-slate-500 font-medium">{html ? 'Файл загружен' : 'Выберите .html файл'}</p>
              </div>
            </div>
          </section>
          <section className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-bold">Переменные</h2>
              <div className="flex gap-2">
                <button onClick={handleAiAnalyze} disabled={!html || isAnalyzing} className="bg-purple-600 text-white px-4 py-2 rounded-xl text-sm font-bold disabled:opacity-50">
                  {isAnalyzing ? 'Анализ...' : 'ИИ Поиск'}
                </button>
                <button onClick={addMapping} className="bg-slate-100 px-4 py-2 rounded-xl text-sm font-bold">Добавить</button>
              </div>
            </div>
            <div className="space-y-4">
              {mappings.map(m => (
                <div key={m.id} className="p-5 border border-slate-200 rounded-2xl space-y-3 relative group">
                  <button onClick={() => removeMapping(m.id)} className="absolute top-4 right-4 text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                  <input type="text" value={m.label} onChange={e => updateMapping(m.id, 'label', e.target.value)} className="w-full border-b border-slate-100 text-sm font-bold outline-none py-1" />
                  <div className="grid grid-cols-2 gap-4">
                    <input type="text" value={m.originalText} onChange={e => updateMapping(m.id, 'originalText', e.target.value)} placeholder="Искать в коде" className="bg-slate-50 p-2 rounded-lg text-xs outline-none" />
                    <input type="text" value={m.replacementText} onChange={e => updateMapping(m.id, 'replacementText', e.target.value)} placeholder="Новое значение" className="bg-blue-50 p-2 rounded-lg text-xs outline-none" />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="lg:col-span-4">
          <button onClick={handleSave} disabled={!name || !html} className="w-full bg-blue-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-blue-200">Опубликовать изменения</button>
        </div>
      </div>
    </div>
  );
...
const Previewer = ({ template, isPublic = false }: { template?: Template | null, isPublic?: boolean }) => {
  ...
  if (!template) return (
    <div className="h-screen flex items-center justify-center flex-col bg-slate-50">
      <div className="p-8 bg-white border border-slate-200 rounded-3xl shadow-sm text-center">
        <h2 className="text-xl font-bold text-red-600 mb-2">Страница не найдена</h2>
        <Link to="/" className="text-blue-600 font-bold hover:underline">Вернуться в панель</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-900">
      <div className="bg-white border-b border-slate-200 px-4 h-14 flex items-center justify-between z-20">
        <div className="flex items-center gap-3">
          {!isPublic && <button onClick={() => navigate('/')} className="p-2 hover:bg-slate-100 rounded-xl"><ArrowLeft className="w-5 h-5" /></button>}
          <span className="text-sm font-bold">{template.name}</span>
        </div>
        <div className="flex gap-2">
          {!isPublic && (
            <button onClick={handleShare} className={`px-3 py-1.5 rounded-xl text-xs font-bold ${copied ? 'bg-green-100 text-green-700' : 'bg-blue-50 text-blue-600'}`}>
...
function AppContent() {
  ...
  const deleteTemplate = (id: string) => {
    if (confirm('Удалить проект?')) setTemplates(prev => prev.filter(t => t.id !== id));
  };
  ...
  return (
    <div className="min-h-screen font-sans">
      <Routes>
        ...
        <Route path="/editor/:id" element={<><Header /><Editor onSave={saveTemplate} initialTemplate={templates.find(t => t.id === useParams().id)} /></>} />
        <Route path="/view/:id" element={<Previewer template={templates.find(t => t.id === useParams().id)} />} />
